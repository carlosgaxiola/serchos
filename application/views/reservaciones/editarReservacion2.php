...
<main class="content-wrapper no-aside">
	<section class="content" id="frm">
		...
		<form method="post" id="reservar2" name="reservar2"
			action="<?php echo base_url("index.php/reservaciones/actualizar") ?>">
			<input type="hidden" name="cantLimit" value="<?php echo $cantMesasDis ?>">
			<input type="hidden" name="paso" value="2">
			<input type="hidden" name="idReservacion" value="<?php echo $reservacion['id'] ?>">
			<?php foreach ($data as $name => $value): ?>
				<input type="hidden"
					name="<?php echo $name ?>"
					value="<?php echo $value ?>">
			...
			<div class="row">
				...
				<div class="form-group col-xs-6
					...
					<div class="col-xs-4">
						...
						<?php 
							$cantidad = $reservacion['cant_mesa'];
...
<script>
	var confirmar = function () {
		BootstrapDialog.confirm({
			title: "Confirmar",
			message: "La reservación sera modficada.<br>¿Desea continuar?",
			btnOKLabel: "Sí",
			btnCancelLabel: "No",
			btnOKClass: "btn-primary",
			callback: function ( ok ) {
				if (ok) {
					document.reservar2.submit();
				}
			}
		})
	}
